...
## Что опробовано
- как отправить простой запрос,
- как работают `GET`, `POST`, `PUT`, `DELETE`, 
- как создавать **переменные** для **коллекции** и для **окружения**, 
- как устанавливать и получать значения из **переменных** в `Scripts`,
- как писать простые **тесты** для `post-response` (также через `snippets`),
- как фильтровать вывод данных через **JSON Server**.
